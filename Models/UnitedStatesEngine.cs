...
{
    public class UnitedStatesEngine : PageHandler, ILocationEngine
    {
        public ModelStateDictionary GetModelErrors(ModelStateDictionary modelState, NameValueCollection form, SafeDictionary postData)
        {
            ...
            var numbersOnlyZipLength = Regex.Replace(billingZip, numbersOnlyRegex, string.Empty).Length;
            ...
            else if (numbersOnlyZipLength != 5 && numbersOnlyZipLength != 9 && validateBillingZip)
            {
                modelState.AddModelError("BillingZip", "A valid Zip is required");
            }
